...
<strong>Дата написания:</strong> 26.04.2021 16:55
</pre>
...
* `linu`
* `z` - указатель того, что файл с ядром сжат (формат сжатия обычно `zlib`. Не всегда используется именно это сжатие, иногда можно встретить `LZMA` или `BZIP2`, а также некоторые другие).
...
> Хочу заострить ваше внимание на том, что оконный менеджер, панель и некоторые другие компоненты можно использовать и отдельно от рабочего окружения. То, что описано в списке выше - далеко не правило.
