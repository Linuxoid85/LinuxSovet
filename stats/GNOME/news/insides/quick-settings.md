...
Это первая статья в данном разделе. И начать я хотел про несколько неподтверждённых новостей. Может быть, изменения, которые я здесь описал, войдёт в будущие релизы GNOME, а может быть и нет. Кто знает?
...
Из него можно изменить яркость экрана, уровень звука, подключиться к WiFi, 3/4G, Bluetooth, etc., здесь переключаются режимы питания... Да даже ПК отсюда выключается. И тут пару месяцев назад предложили её аццкий редизайн. Редизайн такой, что волосы на пипиське дыбом встают. Пиписька, кстати, тоже. Дымом.
