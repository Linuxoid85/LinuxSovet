...
<strong>Дата написания:</strong> 19.03.2022, 23.03.2022 00:00
</pre>

Доброго времени суток! GNOME 42 должен выйти днём/ближе к вечеру уже сегодня, 23 марта 2022 года. В этой версии произошло довольно большое число изменений и улучшений, среди которых можно отметить портирование многих приложений с GTK3 на GTK4 и LibAdwaita, обновление темы Shell, добавление новых страниц параметров в GNOME Control Center и многое другое :)
...
- Часы;
- Калькулятор;
- Расширения (изначально использовал "голый" GTK4);
- Экскурсия;
- Шрифты;
- Символы;
- Календарь;
...
К сожалению, не портированным даже на GTK4 оказался файловый менеджер Nautilus. Да, версия 42.alpha1 была на GTK4 и LibAdwaita, но она была очень нестабильной, да и было много регрессий (не работал drag&drop, копирование в буфер обмена, некорректно отображались иконки файлов, некорректно работал просмотр в виде списка и пр.). Если их исправить - то разработчики просто бы не уложились в поставленные сроки, поэтому было принято решение "откатить" файловый менеджер обратно на GTK3. Зато хоть разработчики обновили т.н. pathbar (панель, где отображается путь до текущей рабочей директории).

Портирование Nautilus на GTK4 и LibAdwaita производится в отдельной ветке. Возможно, в GNOME 43 или 44 мы увидим обновлённую версию Nautilus, но уже не на GTK3.
...
Кроме того, из панели убрали те закругления, к которым мы все давно привыкли.
...
Обновили тему иконок Adwaita. Символьные иконки более элегантны, иконки папок перекрашены в синий цвет. Раньше они были коричневого, иконки архивов были с ними очень схожи. Теперь вы точно не перепутаете папку и архив, называетца :)
...
Задействованы виджеты на базе OpenGL и реализована поддержка аппаратного декодирования видео. Улучшена интеграция с оболочкой GNOME Shell.
...
Оптимизирована отрисовка полноэкранных приложений, что снизило потребление электроэнергии и, возможно, повысило FPS в играх.

Кроме того, из GNOME SDK удалили библиотеку Clutter и связанные с ней компоненты.
...
Программа для создания скриншотов теперь невостребована - вместо неё в сам GNOME Shell встроен новый скриншотер с довольно продвинутыми возможностями:
...
В меню окна (вызывается сочетанием Alt+Space или ПКМ по заголовку окна) добавлена опция "Сделать скриншот". Нетрудно догадаться, что при нажатии на этот пункт меню создаётся скриншот окна :)

![](pic/30.png)

## Заключение

Наше сообщество одним из первых составило эту новость и выложила её в открытый доступ. Я считаю это отличным результатом. Что по поводу GNOME 42 - это весьма хорошее DE, которое должно являться основным во всех популярных дистрибутивах GNU/Linux. Оно самобытно и не пытается копировать Windows, MacOS и прочих. Это есть хорошо.
