...
В функции `write_text()` слегка изменился оператор `ofstream`. Добавился аргумент `ios_base::app`. Он нужен для того, чтобы не затирать содержимое файла до записи в него при его открытии, а записывать новые значения в конец (если этот файл уже существует).
